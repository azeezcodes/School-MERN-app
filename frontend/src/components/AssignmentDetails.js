...
const AssignmentDetails = () => {
	...
	const sendMarks = () => {
		...
		Axios.post(`/submission/grade/${assignmentID}/${currentStudent.student_id}`, {
			"marks" : marks
		})
		.then(res => {
			if(res.data){
	...
	const submitAssignment = () => {
        ...
        .then((res) => {
            Axios.post(`/submission/attachment/${res.data.data._id}`, formData, {
                headers: {
	...
	const checkAttachment = () => {
		Axios.get(`/hasAttachment/${assignmentID}`)
		.then(res => {
			if(res.data.success){
    ...
    const checkHasSubmitted = () => {
        Axios.get(`/hasSubmitted/${assignmentID}/${user._id}`)
        .then((res) => {
            if(res.data.success)
		...
		<div className="course-container">
			<div className="course-heading-block" style={{flexDirection: "row", paddingRight: 0}}>
                <div style={{width: '60%', display: "flex", flexDirection: "column", alignItems: "flex-start", textAlign: "left", marginRight: '5%',}}>
                    ...
                    <br/>
                    <p style={{cursor: "pointer",fontSize: 16, color: '#6C63FF', fontFamily: 'Poppins', fontWeight: 600, margin:0, padding: 0, marginTop: 5}}>
					...
					<reactFragment>
					<br/>
					<a href={`/assignment/attachment/${assignmentID}`} target="_blank">
					<button style={{padding: '8px 15px', marginLeft: 0, marginTop: 0, textAlign: "center"}}>
                ...
                <div className="assignment-upload">	
					<h5 className="heading">{userType === 'student' ? hasSubmitted ? <h6 style={{fontSize: 16, fontWeight: 500, color: '#6C63FF', fontFamily: 'Poppins', textAlign: 'center'}}>Work submitted successfully</h6> : 'My Submission' : 
						...
						userType === 'student' ?
							<React.Fragment>
								<button className="changeColorBG" style={{backgroundColor: "transparent", border: '0px solid #eee', boxShadow: "none", padding: '5px 10px', alignItems: "center", flexDirection: "row", justifyContent: "center", overflow: "hidden", height: 40}}>
									...
									<p className="changeColor" style={{fontSize: 16, fontWeight: 500, color: '#6C63FF', margin:0, fontFamily: 'Poppins'}}>Upload file</p>
								</button>
								{submission ? 
								...
								<button style={{padding: '9px 10px', alignItems: "center", flexDirection: "row", justifyContent: "center", marginTop: 20}} onClick={ submitAssignment } disabled={hasSubmitted}>
						...
						: 	<React.Fragment>
								...
								{studentSubmissions.map((item, index) => { 
									...
									<div  className="student-box" key={index} style={{justifyContent: "space-between"}}>
										...
										<div  style={{display: "flex", flexDirection: "row", alignItems: "center"}}>
										<a href={`/submission/getAttachment/${assignmentID}/${item.student_id}`} target="_blank">
										<Download size={20} className="sub" style={{cursor: "pointer"}}/>
										</a>
										{
											item.marks_obtained ? 
											<div style={{fontSize: 18, fontWeight: "bold", color: "#6C63FF", marginLeft: 20, marginRight: 5}}>{item.marks_obtained}</div> 
											: 
					...
					<span>Pending submissions</span>}</h5>
					{	
						userType === 'student' ?
							null
						: 	<React.Fragment>
			...
			>
				...
				<div style={{position: "absolute", bottom: 25, right: 25, display: "flex", flexDirection: "row-reverse", alignItems: "center"}}>
					...
					<button style={{backgroundColor: 'transparent', boxShadow: 'none'}} onClick={closeModal}>
						<p style={{fontSize: 16, fontWeight: 500, color: '#6C63FF', margin:0, fontFamily: 'Poppins', letterSpacing: 0.8}}>Cancel</p>
					</button>
				</div>
			</Modal>
		</div>
