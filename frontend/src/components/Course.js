...
import {FileText, Edit3, Book, Download, Copy, Plus, X, UserX, ArrowLeft, HelpCircle, Trash2, LogOut, Send } from 'react-feather';
...
const Course = () => {
	...
	React.useEffect(() => {
		Axios.get(`/course/assignment/${courseID}`)
		.then(res => {
			if(res.data.success) {
	...
	React.useEffect(() => {
		...
		Axios.get(`/messages/${loc[loc.length-1]}`)
		.then(res => {
			if(res.data.success) {
	...
	const openAutogradeModal = () => setIsOpenAutograde(true);
	const afterOpenModal = () => {};
	const closeModal = () => setIsOpen(false);
	const closeAutogradeModal = () => setIsOpenAutograde(false);
	...
	const postMaterial = () => {
		...
		Axios.post('/assignment', materialData)
		.then(res => 
			{
				var formData = new FormData();
				formData.append("file", attachment);
				Axios.post(`/assignment/attachment/${res.data.data._id}`, formData, {
				headers: {
	...
	const changeCourseName = () => {
		...
		const url = `/course/changeName/${courseID}`
		Axios.post(url , {
			'name': newCourseName
		})
		.then(res => {	
	...
	return (
		...
		<div className={"background boxshadowtop"} style={{borderTopLeftRadius: 15 , borderTopRightRadius: 15,width: '80%', height: 75, position: 'fixed', bottom: 0, margin:'0 auto', zIndex: 999, display: 'flex', flexDirection: 'row', marginLeft: '20%', padding: '5px 10px', paddingTop: 15}}>
				...
				<button style={{width: 45, height: 45, borderRadius: 25,marginTop:0, marginLeft:0, padding:0, display: 'flex', alignItems: 'center', justifyContent: 'center'}} className="background" onClick={sendMessage}>
					<Send size={23} color={message.length ? "#6C63FF" : '#ababab'} style={{transform:'rotate(45deg)', marginRight:5, transition: '0.2s ease'}}/>
				</button>
		</div> : null 
		}
		...
		<div className="course-container">
						...
						<div style={{display: 'flex', flexDirection: 'row-reverse', alignItems: 'center'}}>
							<Link to={`/home`}>
								<div className="settings-icon">
									<Trash2 size={20} color="#6C63FF" style={{}} onClick={() => {deleteCourse(courseInfo.course_code, courseID);}}/>
								</div>			
							</Link>
							<div className="settings-icon" style={{marginRight: 0}}>
							<Edit3 size={21} color="#6C63FF" style={{cursor: 'pointer'}} className="changeColor" onClick={openCourseNameModal}/>
							</div>
						</div>
					}
				</div>
			...
			<div className="course-heading-block">
				...
				<div className="students-box" style={{marginBottom:0, paddingBottom:0, marginTop:10}}>
					...
					<p className="sub" style={{marginLeft: 70, fontFamily:'Poppins', fontSize: 13.5, color: '#434343', fontWeight: 500, marginTop: 30, letterSpacing:0.3}}>{courseStudents.length ? courseStudents.length : 0} student(s) enrolled</p>
				</div>
			</div>
			...
			<div style={{width: '100%', marginTop: 0}}>
				...
				{posts.filter(p => p.is_assignment === true).length ?
					posts.filter(p => p.is_assignment === true).map((item, index) => {
				...
				{posts.filter(p => p.is_assignment === false).length ?
					posts.filter(p => p.is_assignment === false).map((item, index) => {
							return <Post postType={item.is_assignment ? 'assignment' : 'studymaterial'} title={item.title} info={item.description} assID={item._id}/>
			...
			>
				...
				<div style={{width: '100%', display: "flex", flexDirection: "row", alignItems: "center", marginTop: 30}}>
					<div style={{display: "flex",flexDirection: "row", alignItems: "center"}}>
						<label class={"checkbox-container sub"} style={{borderColor: !isAssignment && !isQuiz ? '#6C63FF' : ''}}>
						...
						{/* <Link to={`/quiznew/${courseID}`}> */}
						<label class={"checkbox-container sub"} style={{borderColor: isQuiz ? '#6C63FF' : ''}}>
				...
				<button className='changeColorBG' style={{backgroundColor: "transparent", border: '0px solid #eee', boxShadow: "none", padding: '5px 10px', alignItems: "center", flexDirection: "row", justifyContent: "center", overflow: "hidden", height: 40,}}>
					...
					<p className="changeColor" style={{fontSize: 15, fontWeight: 700, color: '#6C63FF', margin:0, fontFamily: 'Mulish'}}>Upload file</p>
				</button>
				}
				...
				<div style={{bottom: 10, right: 10, display: "flex", flexDirection: "row-reverse", alignItems: "center"}}>
					...
					<button style={{boxShadow: 'none', backgroundColor: 'transparent'}} onClick={closeModal}>
						<p style={{fontSize: 16, fontWeight: 600, color: '#6C63FF', margin:0, fontFamily: 'Poppins', letterSpacing: 0.8}}>Cancel</p>
					</button>
				</div>
				...
				<div>
					...
					<div style={{bottom: 25, right: 25, display: "flex", flexDirection: "row-reverse", alignItems: "center"}}>
