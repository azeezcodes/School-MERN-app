                ...
                <div style={{display: 'inline-block', width: '100%', height: 500, marginTop: 40}}>
                    {
                        notes.map((note, index) => {
                            return (
                                <NoteBox date={note.createdAt} content={note.content} id={note._id} key={index} deleteNote={(id) => deleteNote(id)}/>
                            )
                        })
                    }   
                </div> 
